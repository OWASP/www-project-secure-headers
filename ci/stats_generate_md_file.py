...
Source:
    ...
    https://github.com/OWASP/www-project-secure-headers/tree/master/subprojects/statistics
"""
import hashlib
import json
import re
...
def csp_contain_unsafe_expression(csp_policy):
    ...
    for directive in directives:
        if len(re.findall(exp_directive_name_allowing_all_unsafe_expressions, directive)) > 0 and len(re.findall(exp_all_unsafe_expressions, directive)) > 0 or directive.strip().startswith("style-src") and len(re.findall(exp_style_unsafe_expressions, directive)) > 0:
...
def compute_secure_headers_global_usage():
    ...
    description = "Provide the distribution of usage of secure headers across all domains analyzed."
...
def compute_csp_using_directives_with_unsafe_expressions_configuration_global_usage():
    ...
    title = "Global usage of content security policy with directives allowing unsafe expressions"
    description = "Provide the distribution of content security policy allowing unsafe expressions across all domains analyzed.\n\nDetermine if a CSP policy contains `(default-src|script-src|script-src-elem|script-src-attr|style-src)` directives using `(unsafe-inline|unsafe-hashes|unsafe-eval)` expressions.\n\nBased on [Report-URI CSP](https://report-uri.com/home/generate) generator allowed instructions for CSP directives."
