...
💡 Additional information about technical information disclosure in HTTP header on [OpenCRE](https://www.opencre.org/cre/233-748?name=OWASP+Secure+Headers+Project&section=Prevent+information+disclosure+via+HTTP+headers&link=https%3A%2F%2Fgithub.com%2FOWASP%2Fwww-project-secure-headers%2Fblob%2Fmaster%2Fmainsite%2F03_best_practices.md%23prevent-information-disclosure-via-http-headers).
...
| TeamCity-Node-Id                                                                                                                                              | `MAIN_SERVER`                                    | Indicate the presence of the [JetBrains TeamCity](https://www.jetbrains.com/teamcity/) CI/CD platform instance.                                                                                                                                            |

<!-- HEADERS_REMOVE_TABLE_END -->

## Prevent exposure to cross-site scripting when hosting uploaded files

This section describes, how the HTTP response header named [Content-Disposition](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Disposition), can be used to prevent exposure to cross-site scripting when hosting uploaded files and opening them in the same web browsing context as the application.

There can be a case in which an application allows a user to upload a file and then allows this file to be accessed by other users. If such a feature allows uploading of HTML files (also applies for [SVG files](https://hackerone.com/reports/1276742)) then it can be used, as a vector, to store an HTML file containing JavaScript code. Therefore, the feature become prone to [stored cross-site scripting](https://portswigger.net/web-security/cross-site-scripting/stored) vulnerability.

To prevent this exposure, the HTTP response header named [Content-Disposition](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Disposition), can be used with the following value to instruct browsers to download the file instead of open it in the same web browsing context as the application:
...
* 💡 If the web framework/web server you are using provides CORS features then always leverage them instead of implementing it manually:
...
The tool [nuclei](https://github.com/projectdiscovery/nuclei) can be used, via the template named [cors-misconfig](https://github.com/projectdiscovery/nuclei-templates/blob/main/http/vulnerabilities/generic/cors-misconfig.yaml), to test a CORS configuration.
...
📺 This [demonstration video](assets/misc/demo_information_disclosure_via_browser_file_caching.mp4) shows an example, of such information disclosure, via a file cached by the browser.
...
* `max-age=0`: Is used to force the expiration of any cached version of the resources associated with the response.
...
🚩 Not every **[directive](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy#directives)** falls back to the **[default-src](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/default-src)** directive when it is not specified in the CSP policy.
...
📺 This [demonstration video](assets/misc/demo_csp_bypass_due_to_no_form_action_directive.mp4) shows an example.
...
📺 This [demonstration video](assets/misc/demo_csp_bypass_due_to_no_frame_ancestors_directive.mp4) shows an example.
...
💡 Based on tests performed, modern browsers support a sufficient size to specify a large CSP policy in case of need.
